...
### 1-6. Homeを並び替えても、再生キューは元の順番のままだった問題

**何が起きたか**

Homeに「タイトル順」「追加日順」の並び替え機能を追加したとき、実装直後の動作確認で
「並び替えた見た目の順番」と「曲を再生したときのUp Nextの順番」が食い違うことに気づきました。
例えば「タイトルA→Z順」に並び替えて一番上の曲を再生しても、Up Nextには元々の（並び替え前の）
順番の続きが入ってしまう、という状態でした。

**なぜ起きたか**

再生キュー（History / Current / Up Next）を管理する`useAudioPlayer`フックには、
Home画面が現在どんな並び順で表示されているか関係なく、**APIから取得した生の曲一覧
（`mediaItems`）をそのまま渡していました**。
...
`useAudioPlayer`内部では、渡された配列の並び順をそのまま使って「今の曲より後ろの曲」を
Up Nextとして組み立てます（`playableAudioItems.slice(startIndex + 1)`）。そのため、
Home画面の**表示用**の並び替え（`sortItems`でソートした配列）と、**実際に渡していた
配列**が別物になっており、見た目の順番と再生順がズレていました。

**どう直したか**

「Homeでの現在の並び順を適用した配列」を先に作り、それを`useAudioPlayer`にも
Home表示にも共通で使うようにしました。

```ts
// 曲の種別（Audio/Video）では絞り込まない。タブを切り替えても再生中Audioの
// キューは保持したいため、両方を含めたままソートだけ適用する。
...
// Home表示用は、これをさらにmediaTypeで絞り込むだけ。
...
ポイントは、`mediaType`（Audio / Video）による絞り込みは`useAudioPlayer`に渡す
**前**ではなく**後**（Home表示専用の`filteredItems`を作る段階）で行うことです。
`useAudioPlayer`側には常にAudio・Video両方を含んだ（ソート済みの）配列を渡し、
内部の`type === "audio"`フィルタに任せます。もし先に`mediaType`で絞り込んでから
`useAudioPlayer`に渡すと、Videoタブを見ている間はAudioの再生キューが空になって
しまうという、別の壊れ方をします（実際に一度この形で書きかけて気づきました）。

**教訓**

> 同じ元データ（この場合は曲一覧）を**複数の画面・機能が別々の目的で使っている**とき、
> 一方だけ加工（フィルタ・ソート等）を加えると、もう一方との整合性が静かに崩れることがある。
> 「ユーザーから見て同じ並び順であるべきものが、実は内部では別の配列を参照していないか」を
> 機能追加のたびに確認する。特に、一覧の表示順を変える機能（ソート・フィルタ・検索）を
> 追加するときは、その一覧から派生する他の機能（今回で言う再生キュー）が古い順序基準の
> ままになっていないか、影響範囲を洗い出してから実装する。

---

### 1-7. タイトルを編集しても、開いているDetail Sheetの表示だけ古いまま

**何が起きたか**

曲のタイトル編集機能を追加したとき、Detail Sheetを開いたまま編集→保存すると、
保存自体は成功してHomeの一覧には新しいタイトルが反映されるのに、**その場で開いている
Detail Sheet自体の表示だけ古いタイトルのまま**という不具合がありました。

**なぜ起きたか**

Detail Sheetを開くとき、「どの曲の詳細を表示するか」を`MediaItem`オブジェクトそのもの
（曲一覧の1要素のコピーではなく参照ですが、その時点でのスナップショット）として
`useState`に保存していました。
...
// ...
onShowDetail={setDetailItem}  // 曲一覧のitemオブジェクトをそのままstateへ
```

タイトル更新後、親コンポーネント（`App.tsx`）側の曲一覧`mediaItems`は新しい配列
（新しいタイトルを持つ新しいオブジェクト）に置き換わりますが、`Library`内部の
`detailItem`は**更新前の古いオブジェクトを指したまま**なので、Reactの目には
「別に何も変わっていない」ように見え、再レンダリングされても古いタイトルが表示され
続けていました。

**どう直したか**

最初は「`items`が変わるたびに`detailItem`を最新のものへ同期するuseEffect」を書きましたが、
そもそも同じデータを2箇所（`items`本体と`detailItem`という名の"コピー"）で持っていることが
問題の本質でした。**オブジェクトそのものではなく、IDだけをstateに持ち、表示のたびに
一覧から検索して取り出す**形に直したところ、同期用のuseEffectごと不要になりました。

```ts
// 修正後: IDだけを保持し、実際のitemは毎回itemsから引く
const [detailItemId, setDetailItemId] = useState<MediaItem["id"] | null>(null)
const detailItem =
  (detailItemId ? items.find((item) => item.id === detailItemId) : null) ?? null
```

こうすることで、`items`が更新されるたびに`detailItem`も自動的に最新の内容になります
（削除されて見つからなくなった場合は自然に`null`になるので、Sheetを閉じる処理も
別途書く必要がなくなりました）。

**教訓**

> 「一覧の中の1件を選んで詳細表示する」ようなUIでは、選択中の**データそのもの**を
> stateに持つと、元データが更新されたときに古いスナップショットを表示し続けるバグの
> 温床になる。選択中の**ID（キー）だけ**をstateに持ち、表示する実体は毎回
> `items.find(...)`のように元データから導出する方が、同期漏れが原理的に起こらず
> シンプルになる。「stateを同期させるためのuseEffect」を書きたくなったときは、
> 一度「そもそもstateを2つに分けず、片方を導出値にできないか」を疑うとよい
> （Reactの公式ドキュメントでも "You Might Not Need an Effect" として同様の指針がある）。
...
- [ ] **一覧の並び替え・フィルタ・検索機能を追加するとき**
  - その一覧から派生している他の機能（再生キュー、統計、エクスポート等）が、
    古い（並び替え前の）順序や絞り込み前のデータをまだ参照していないか？
  - 「一覧の表示順」と「その一覧を使う機能の内部での順序」が同じデータソースを
    参照する構造になっているか（表示用と機能用で別々の配列を持っていないか）？

- [ ] **「一覧から1件選んで詳細を表示・編集する」ようなUIを書くとき**
  - 選択中の対象を、データそのもの（オブジェクト）ではなくIDだけをstateに持ち、
    実体は`items.find(...)`などその場で導出しているか？
  - 元データが更新された後も、表示中の詳細画面がその更新を反映するか確認したか？
    （保存直後にその場で表示が更新されない、というバグは典型パターン）
